...
def locale(bot, update, args):
    ...
    if len(args) > 0:
        ...
        if locale in list_locales:
            if locale in ('en', 'ru', 'ua', 'es', 'tr', 'id','hi'):
...
def locale_button(bot, update):
    ...
    lang_match = re.findall(r"en|ru|ua|es|tr|hi|id", query.data)
    if lang_match:
        if lang_match[0]:
    ...
    query.message.reply_text(text, parse_mode=ParseMode.MARKDOWN,
                                            ...
                                            InlineKeyboardButton("Indonesian 🇮🇩", callback_data="set_lang_id"),InlineKeyboardButton("Hindi 🇮🇳", callback_data="set_lang_hi")]] + [[
